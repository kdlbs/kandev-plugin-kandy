...
| affection fades (v0.11.0) | **−5 per full elapsed day** while temperament > 0 and the last effective treat is > 24h old, **clamped at 0** | the mirror of "time heals", sharing its lazy accrual and `last_passive_heal_at` checkpoint (measured from max(`last_pet_effect_at`, checkpoint)). Neglect walks a bond back toward neutral and **stops there** — it can never make a kandy wary or fearful; only a bucket does that. Pace: beloved (+30) → content in ~5 days, neutral in ~7; holding beloved costs ~5 treats/day. Migration: a fond ledger with no checkpoint gets one stamped *now*, so upgrading never retro-fades a bond earned before the rule existed |
