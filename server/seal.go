...
//     verifyLoadedLedger; see loadLedger.
//
...
// use and creating it when the vault has none yet. The decoded key is
// cached for the process lifetime (callers hold p.mu). created is true only
// when this call minted the key — the grandfather signal. Any vault error
// returns err: callers must treat that as "infrastructure, not fraud".
func (p *plugin) ensureSealKey(ctx context.Context, host pluginsdk.Host) (key []byte, created bool, err error) {
	if p.sealKey != nil {
		return p.sealKey, false, nil
	}
	hexKey, found, err := host.GetSecret(ctx, secretKeyLedgerHMAC)
	if err != nil {
	...
	if err := host.SetSecret(ctx, secretKeyLedgerHMAC, hex.EncodeToString(raw)); err != nil {
		return nil, false, err
	}
	p.sealKey = raw
	return raw, true, nil
...
// and re-verify on the next load". Callers must hold p.mu.
func (p *plugin) verifyLoadedLedger(ctx context.Context, host pluginsdk.Host, l *ledger) (*ledger, bool) {
	...
	if err := host.SetState(ctx, stateScope, "", stateKey, ledgerToMap(reborn)); err != nil {
